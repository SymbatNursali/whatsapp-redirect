...
/**
 * Серверное событие Contact — отправляется только при клике по кнопке.
 * Используется для дедупликации с клиентским пикселем через eventId.
 */
export async function trackClickAction(sourceSlug: string, eventId: string) {
  ...
  const url = `https://${headerList.get("host")}/r/${sourceSlug}`;
